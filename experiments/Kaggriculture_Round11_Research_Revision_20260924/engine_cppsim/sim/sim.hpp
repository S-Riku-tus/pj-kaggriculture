...
struct Farm {
    ...
    int32_t tel_sell_zero_fill = 0;  // SELL orders that delivered nothing: a market slot burned
    int32_t tel_buy_zero_fill = 0;   // BUY orders that delivered nothing: the same, for purchases

    // EFFICIENCY ledger (2026-09-14): value the farm HAD and did not use. Not a
    // loss the engine inflicts, an opportunity the agent left on the table,
    // counted at the same sites so the two ledgers can be read together. The
    // user's rule that made it: escapes are one channel of many, and an
    // instrument that measures one channel invites tuning to that channel.
    int32_t tel_tile_cap_units = 0;      // yield accrual clipped at a tile's holding cap: not harvested in time
    int32_t tel_fertilizer_forgone = 0;  // animal-days whose fertilizer was on offer and never collected
    int32_t tel_care_unfed = 0;          // CARE without FEED the same day: the bonus never accrues
    int32_t tel_animal_shed_days = 0;    // animals still in the shed at end of day: capital idle
    int32_t tel_idle_tile_days = 0;      // unlocked tiles empty or weed at end of day: land idle
    int32_t tel_weed_tile_days = 0;      // of those, weeds nobody dug
    int32_t tel_hand_pass_turns = 0;     // hired hands that did nothing this turn, explicit PASS or no order at all
    int32_t tel_farmer_pass_turns = 0;   // the farmer's PASS turns, kept apart: the farmer is free, a hand costs a day
    int32_t tel_phantom_hand_actions = 0;// actions addressed to hands the farm does not have
    int32_t tel_move_dead = 0;           // a move off the board
    int32_t tel_locked_dead = 0;         // a tile action on locked land
    int32_t tel_pickup_dead = 0;         // PICKUP away from the shed, or of what the shed lacks
    int32_t tel_drop_dead = 0;           // DROP away from the shed
    int32_t tel_place_dead = 0;          // PLACE with nothing to place, or nowhere to put it
    int32_t tel_build_dead = 0;          // BUILD on a tile that is not empty
    int32_t tel_dig_dead = 0;            // DIG on an empty tile, or on an animal
    int32_t tel_fertilize_dead = 0;      // FERTILIZE on a non-plant, or with none carried
    int32_t tel_collect_dead = 0;        // COLLECT_FERTILIZER with none available
    int32_t tel_care_dead = 0;           // CARE on no animal, or twice in a day
    int32_t tel_sell_floor_units = 0;    // units sold at the price floor of 1
    int32_t tel_hire_premium = 0;        // the same-day hire ladder above one coin a hand. Hands leave at
                                         // midnight, so this is the price of labour beyond the first hand
                                         // each day, not an avoidable premium: read it as cost structure

    // Silent-leak telemetry, added 2026-09-14 for the failure discussion 741235
    // named "No Cow Left Behind": an agent loses animals on day 1 and day 8 and
    // nothing in the observation says so. The engine never reports a refusal, a
    // starvation or a rotted crop; the bank simply comes out lower, which is the
    // hardest kind of defect to find and the reason to count it at the source.
    // Two families, and the distinction is the point.
    //
    // A. ACTIONS THAT SILENTLY DID NOTHING. The action was issued, the engine
    //    took it and changed no state. `feed_no_wheat` is the exact shape of the
    //    741235 bug: the hand reaches the animal and FEEDs, and the FEED is a
    //    no-op because the wheat never reached that unit's inventory. It is the
    //    PRECURSOR, visible a day or two before the animal is gone.
    int32_t tel_feed_no_wheat = 0;   // FEED on a live hungry animal, carrier had no WHEAT
    int32_t tel_feed_no_animal = 0;  // FEED at a tile holding no animal (often one already escaped)
    int32_t tel_feed_redundant = 0;  // FEED on an animal already fed today (a wasted turn)
    int32_t tel_water_dead = 0;      // WATER on a non-plant or an already-watered tile
    int32_t tel_harvest_dead = 0;    // HARVEST that took nothing (empty, weed, unripe, zero yield)
    int32_t tel_plant_dead = 0;      // PLANT refused: no seed held, or the tile was not empty
    //
    // B. VALUE THAT SILENTLY DISAPPEARED. State the engine destroyed with no
    //    message. Capital is the purchase price of what was lost, so the leak is
    //    denominated in the same coins as the bank.
    int32_t tel_animals_escaped = 0;    // animals gone at consecutive_unfed >= 2
    double  tel_escape_capital = 0;     // their purchase price, coins
    int32_t tel_escape_yield_units = 0; // product held on the tile and lost with it
    int16_t tel_escape_first_day = -1;  // the day the FIRST one went, or -1
    int32_t tel_animal_unfed_days = 0;  // animal-days ending unfed: the runway to an escape
    int32_t tel_plants_dry = 0;         // plants turned to WEED by two dry days
    double  tel_dry_seed_cost = 0;      // their seed price, coins
    int32_t tel_dry_yield_units = 0;    // ripe product lost with them
    int16_t tel_dry_first_day = -1;     // the day the FIRST one went, or -1
    int32_t tel_plant_unwatered_days = 0; // plant-days ending unwatered
    int32_t tel_decay_units = 0;        // yield units rotted off a plant left standing
    int32_t tel_plants_decayed = 0;     // plants that rotted all the way to WEED
...
private:
    ...
    void apply_unit_actions(int p, const Action& a, int day) {
        ...
        if (a.n_units > f.n_units) f.tel_phantom_hand_actions += a.n_units - f.n_units;
        for (int i = 1; i < f.n_units; ++i)          // a hired hand with no order is a hand that passed
            if (i >= a.n_units || a.units[i].op == OP_PASS) f.tel_hand_pass_turns += 1;
        if (a.n_units == 0 || a.units[0].op == OP_PASS) f.tel_farmer_pass_turns += 1;
        bool blocked[N_CROPS];
        for (int c = 0; c < N_CROPS; ++c) blocked[c] = demand[c] > f.seeds[c];

        for (int i = 0; i < n; ++i) {
            UnitAction u = a.units[i];
            if (u.op == OP_PLANT && u.arg < N_CROPS && blocked[u.arg]) {
                // Dropped here, before the action is ever applied, so the
                // counter belongs here too: asking to plant more of a crop than
                // the farm holds seed for loses EVERY plant of that crop this
                // turn, not just the surplus, and nothing anywhere says so.
                f.tel_plant_dead += 1;
                continue;
            }
    ...
    void apply_unit(Farm& f, int idx, const UnitAction& u, int day) {
        ...
        switch (u.op) {
            ...
            case OP_NORTH: case OP_SOUTH: case OP_EAST: case OP_WEST: {
                ...
                if (nx < 0 || nx >= bs || ny < 0 || ny >= bs) { f.tel_move_dead += 1; return; }
        ...
        if (u.op == OP_DROP) {
            if (!is_shed_adjacent(fx, fy, bs)) { f.tel_drop_dead += 1; return; }
        ...
        if (u.op == OP_PICKUP) {
            if (!is_shed_adjacent(fx, fy, bs)) { f.tel_pickup_dead += 1; return; }
            if (u.n <= 0 || u.arg >= N_ITEMS) { f.tel_pickup_dead += 1; return; }
            int take = std::min<int>(u.n, f.shed[u.arg]);
            if (take <= 0) { f.tel_pickup_dead += 1; return; }
            f.shed[u.arg] -= take; f.shed_total -= take;
            f.inv_add(idx, u.arg, take);
            return;
        }
        if (u.op == OP_PLACE) {
            if (u.arg >= N_ITEMS) { f.tel_place_dead += 1; return; }
            if (is_animal(u.arg)) {
                ...
                if (tile.kind == want && !tile.has_animal) {
                    if (f.inv_take(idx, u.arg, 1)) {
                        ...
                        f.tel_place_dead += 1;         // the right structure, nothing carried
                    }
                    return;
                }
            }
            // Not an animal on its structure: a PLACE next to the shed puts the
            // carried item into the shed, and anywhere else it does nothing.
            if (is_shed_adjacent(fx, fy, bs)) {
                int n = std::min<int>(u.n, inv[u.arg]);
                if (n <= 0) { f.tel_place_dead += 1; return; }
        ...
        if (tile.kind == T_LOCKED) { f.tel_locked_dead += 1; return; }

        switch (u.op) {
            case OP_PLANT: {
                if (u.arg >= N_CROPS || tile.kind != T_EMPTY || f.seeds[u.arg] <= 0) {
                    f.tel_plant_dead += 1;
                    return;
                }
                f.seeds[u.arg] -= 1;
            ...
            case OP_WATER: {
                if (tile.kind != T_PLANT || tile.watered_today) { f.tel_water_dead += 1; return; }
                ...
                if (!cd.ongoing) {
                    ...
                    if (age >= w0 && age <= cd.max_yield_day) {
                        ...
                        int want = tile.yield_units + bonus;
                        int got = std::min(cd.max_yield, want);
                        f.tel_tile_cap_units += want - got;
                        tile.yield_units = static_cast<int8_t>(got);
                    }
                }
                return;
            }
            case OP_HARVEST: {
                if (tile.kind == T_EMPTY || tile.kind == T_WEED) { f.tel_harvest_dead += 1; return; }
                if (tile.yield_units <= 0) { f.tel_harvest_dead += 1; return; }
                if (tile.kind == T_PLANT) {
                    const CropDef& cd = CROPS[tile.what];
                    if (day - tile.planted_day < cd.first_yield_day) { f.tel_harvest_dead += 1; return; }
            ...
            case OP_FERTILIZE: {
                if (tile.kind != T_PLANT) { f.tel_fertilize_dead += 1; return; }
                if (!f.inv_take(idx, FERTILIZER, 1)) { f.tel_fertilize_dead += 1; return; }
                tile.fertilized_until_day = std::max<int16_t>(tile.fertilized_until_day,
                                                             static_cast<int16_t>(day + 2));
                return;
            }
            case OP_DIG: {
                if (tile.kind == T_EMPTY) { f.tel_dig_dead += 1; return; }
                if (tile.has_animal) { f.tel_dig_dead += 1; return; }   // animals are not removable
                Tile e{}; e.kind = T_EMPTY; tile = e;
                return;
            }
            case OP_BUILD_COOP:
                if (tile.kind != T_EMPTY) { f.tel_build_dead += 1; return; }
            ...
            case OP_BUILD_PASTURE:
                ...
                return;
            case OP_FEED:
                if (!tile.has_animal) { f.tel_feed_no_animal += 1; return; }
                if (tile.fed_today) { f.tel_feed_redundant += 1; return; }
                if (!f.inv_take(idx, WHEAT, 1)) { f.tel_feed_no_wheat += 1; return; }
                tile.fed_today = true;
                return;
            case OP_COLLECT_FERTILIZER:
                if (!tile.has_animal || !tile.fertilizer_available) { f.tel_collect_dead += 1; return; }
                tile.fertilizer_available = false;
                f.inv_add(idx, FERTILIZER, 1);
                f.produced[FERTILIZER] += 1;
                return;
            case OP_CARE:
                if (!tile.has_animal || tile.cared_today) { f.tel_care_dead += 1; return; }
    ...
    // ------------------------------------------------------------ market
    struct OState { uint8_t type; uint8_t item; int32_t remaining; bool live; int32_t asked; };
            ...
            for (int p = 0; p < 2; ++p) {
                ...
                if (i < nq[p]) {
                    ...
                    if (o.op == M_HIRE || o.op == M_BUY_LAND) {
                        os[p].type = o.op; os[p].live = true; os[p].remaining = 1; os[p].asked = 1;
                    } else if (o.op != M_NONE && o.n > 0) {
                        os[p].type = o.op; os[p].item = o.item; os[p].remaining = o.n;
                        os[p].asked = o.n; os[p].live = true;
                    }
                }
            }
            ...
            for (;;) {
                ...
                for (int p = 0; p < 2; ++p) {
                    ...
                    if (commit_unit(q[p].type, q[p].item, q[p].price, st.farms[p])) {
                        ...
                        Farm& fp = st.farms[p];
                        switch (os[p].type) {
                            case M_SELL:        fp.tel_sell_dead += os[p].remaining;
                                                // An order that delivered NOTHING is the
                                                // informative case: it burned one of the ten
                                                // market slots for nothing. The undelivered
                                                // UNITS do not say that, because the field's
                                                // usual "sell everything" idiom asks for 1000
                                                // and means "whatever is there", so a healthy
                                                // agent still books hundreds of dead units.
                                                if (os[p].remaining == os[p].asked) fp.tel_sell_zero_fill += 1;
                                                break;
                        ...
                        // Refused UNITS carry the same ambiguity as undelivered SELL
                        // units: "buy as much wheat as I can afford" asks for a large
                        // quantity on purpose. An order that bought NOTHING is the
                        // unambiguous event, and it usually means the financing
                        // chain broke: the purchase this turn depended on a sale that
                        // did not happen.
                        if (os[p].type != M_SELL && os[p].remaining == os[p].asked) fp.tel_buy_zero_fill += 1;
                        os[p].live = false;
                    }
                }
                if (!committed) break;
    ...
    bool commit_unit(uint8_t op, uint8_t item, int price, Farm& f) {
        switch (op) {
            case M_SELL:
                ...
                f.money += price;
                if (price <= 1) f.tel_sell_floor_units += 1;
                f.sold_units[item] += 1;
    ...
    void do_hire(Farm& f) {
        ...
        f.tel_hire_premium += cost - cfg.hire_mult * fib(0);
        f.hires_today += 1;
    ...
    void decay_plants(Farm& f, int step_i) {
        ...
        for (int y = 0; y < cfg.board_size; ++y)
            for (int x = 0; x < cfg.board_size; ++x) {
                ...
                t.yield_units -= 1;
                f.tel_decay_units += 1;    // ripe product rotting on a tile nobody harvested
                if (t.yield_units <= 0) { Tile w{}; w.kind = T_WEED; t = w; f.tel_plants_decayed += 1; }
            }
    }
    ...
    void end_of_day(int day) {
        ...
        for (int p = 0; p < 2; ++p) {
            ...
            spawn_weeds(f, rng);
            drop_inventories(f);
            f.tel_animal_shed_days += f.shed[GOOSE] + f.shed[COW] + f.shed[SHEEP];
            for (int y = 0; y < cfg.board_size; ++y)
                for (int x = 0; x < cfg.board_size; ++x) {
                    TileKind k = f.tiles[y][x].kind;
                    if (k == T_EMPTY || k == T_WEED) f.tel_idle_tile_days += 1;
                    if (k == T_WEED) f.tel_weed_tile_days += 1;
                }
            int h = cfg.board_size / 2;
            f.n_units = 1;
            f.pos_x[0] = static_cast<int8_t>(h - 1);
    ...
    void daily_refresh_plants(Farm& f, int day) {
        ...
        for (int y = 0; y < cfg.board_size; ++y)
            for (int x = 0; x < cfg.board_size; ++x) {
                ...
                if (!was_watered) f.tel_plant_unwatered_days += 1;
                if (t.consecutive_dry >= 2) {
                    f.tel_plants_dry += 1;
                    f.tel_dry_seed_cost += CROPS[t.what].seed;
                    f.tel_dry_yield_units += std::max<int>(0, t.yield_units);
                    if (f.tel_dry_first_day < 0) f.tel_dry_first_day = static_cast<int16_t>(day);
                    Tile w{}; w.kind = T_WEED; t = w; continue;
                }
                ...
                int want = t.yield_units + (fert ? 2 : 1);
                int got = std::min(cd.max_yield, want);
                f.tel_tile_cap_units += want - got;      // ripe on the plant, nobody came
                t.yield_units = static_cast<int8_t>(got);
    ...
    void daily_refresh_animals(Farm& f, int day) {
        ...
        for (int y = 0; y < cfg.board_size; ++y)
            for (int x = 0; x < cfg.board_size; ++x) {
                ...
                if (!t.fed_today) f.tel_animal_unfed_days += 1;
                if (t.consecutive_dry >= 2) {           // escapes; structure remains
                    f.tel_animals_escaped += 1;
                    f.tel_escape_capital += ANIMALS[t.what - GOOSE].cost;
                    f.tel_escape_yield_units += std::max<int>(0, t.yield_units);
                    if (f.tel_escape_first_day < 0) f.tel_escape_first_day = static_cast<int16_t>(day);
                    TileKind k = t.kind;
                    Tile s{}; s.kind = k; t = s;
                    continue;
                }
                ...
                if (since >= 0 && since % ad.interval == 0) {
                    ...
                    int want = t.yield_units + 1 + bonus;
                    int got = std::min(ad.max_held, want);
                    f.tel_tile_cap_units += want - got;  // the animal held its cap, nobody milked it
                    t.yield_units = static_cast<int8_t>(got);
                    t.pending_care_bonus = 0;
                }
                if (t.cared_today && t.fed_today) t.pending_care_bonus += 1;
                if (t.cared_today && !t.fed_today) f.tel_care_unfed += 1;
                if (t.fertilizer_available) f.tel_fertilizer_forgone += 1;   // yesterday's unit, never collected
