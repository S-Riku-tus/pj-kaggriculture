...
settle sites:
    ...
    {"sell_dead_units": ..,        # SELL units refused: shed empty, full
     ...
     "sell_zero_fill": ..,         # SELL orders that delivered nothing at all
     "buy_zero_fill": ..,          # BUY orders that delivered nothing at all
...
A second family, added in 0.5.0, covers the losses that happen on the
FARM rather than at the market, because those are the ones an agent
author cannot see at all. The engine never reports a starved animal or a
rotted crop: the tile is simply different the next morning.

    {"feed_no_wheat": ..,          # FEED on a hungry animal, carrier held no WHEAT
     "feed_no_animal": ..,         # FEED at a tile with no animal (often one already gone)
     "feed_redundant": ..,         # FEED on an animal already fed today
     "water_dead": ..,             # WATER on a non-plant or an already-watered tile
     "harvest_dead": ..,           # HARVEST that took nothing
     "plant_dead": ..,             # PLANT dropped: no seed, or the tile was not empty
     "dead_actions": ..,           # the six above, summed
     "animals_escaped": ..,        # animals gone at two consecutive unfed days
     "escape_capital": ..,         # their purchase price, in coins
     "escape_yield_units": ..,     # product held on the tile and lost with it
     "escape_first_day": ..,       # the day the first one went, or -1
     "animal_unfed_days": ..,      # animal-days ending unfed: the runway to an escape
     "plants_dry": ..,             # plants turned to WEED by two dry days
     "dry_seed_cost": ..,          # their seed price, in coins
     "dry_yield_units": ..,        # ripe product lost with them
     "dry_first_day": ..,          # the day the first one went, or -1
     "plant_unwatered_days": ..,   # plant-days ending unwatered
     "decay_units": ..,            # yield units rotted off a plant left standing
     "plants_decayed": ..,         # plants that rotted all the way to WEED
     "silent_loss_coins": ..,      # escape_capital + dry_seed_cost
     "silent_loss_units": ..}      # every unit lost to escape, drought, rot and the shed cap

A third family is EFFICIENCY: not a loss the engine inflicts but value the
farm had and did not use, counted at the same sites. And a fourth is what the
season stranded, read from the state at the last step.

    {"fertilizer_forgone": ..,     # animal-days whose fertilizer was on offer, never collected
     "tile_cap_units": ..,         # yield clipped at a tile's holding cap: not harvested in time
     "care_unfed": ..,             # CARE without FEED the same day: the bonus never accrues
     "animal_shed_days": ..,       # animals still in the shed at the end of a day
     "idle_tile_days": ..,         # unlocked tiles empty or weed at the end of a day
     "weed_tile_days": ..,         # of those, weeds nobody dug
     "hand_pass_turns": ..,        # hired hands that did nothing this turn
     "farmer_pass_turns": ..,      # the farmer's PASS turns, kept apart: the farmer is free
     "phantom_hand_actions": ..,   # orders to hands the farm does not have
     "sell_floor_units": ..,       # units sold at the price floor of 1
     "hire_premium": ..,           # the same-day hire ladder above one coin a hand: hands leave
                                   # at midnight, so this is the price of labour, not a waste
     "move_dead", "locked_dead", "pickup_dead", "drop_dead", "place_dead",
     "build_dead", "dig_dead", "fertilize_dead", "collect_dead", "care_dead",
     "shed_units_now": ..,         # units in the shed at the moment of reading
     "shed_value_now": ..,         # their value at current prices
     "animals_in_shed_now": ..,    # animals bought and not yet placed
     "seeds_unplanted_now": ..,    # seeds in the bag
     "seeds_unplanted_cost_now": ..}

The distinction between the two halves of that block is the useful part.
`animals_escaped` is the loss; `feed_no_wheat` and `animal_unfed_days`
are its PRECURSORS, and they are visible a day or two earlier. An agent
that reads them can act while the animal is still there.

Read `sell_dead_units` with care and prefer `sell_zero_fill` to it. The
field's ordinary idiom for "sell everything I have" is to ask for 1,000
units and let the settle fill what exists, so a healthy agent books tens
of thousands of undelivered units and none of them is a loss. An order
that delivered NOTHING is different: it burned one of the ten market
slots that turn, and that is what `sell_zero_fill` counts.

That is not a hypothetical. The competition's discussion 741235, "No Cow
Left Behind", is a public report of agents losing animals on day 1 and
day 8 with nothing in the observation to say why; the two mechanisms
turned out to be a day-0 cash knife-edge and a day-7 wheat stock-out, and
both show as a `feed_no_wheat` before they show as an escape.
