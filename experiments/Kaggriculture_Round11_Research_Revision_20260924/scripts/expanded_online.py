...
ap.add_argument('--battle-zip',required=True,type=pathlib.Path)
ap.add_argument('--output',type=pathlib.Path,default=ROOT/'recomputed_online')
args=ap.parse_args();P=ROOT/'inputs/prior_audit';O=args.output;O.mkdir(parents=True,exist_ok=True)
from market_reconstruction import price
source=(ROOT/'inputs/agents/B1.py').read_bytes();a=ast.parse(source)
...
with zipfile.ZipFile(args.battle_zip) as z:
