# Modified from "Kaggriculture V45: First-Turn Wheat Round Trip" (Ahmed Berat Ozer, Apache-2.0): 10-unit first-turn
# wheat round trip, step-1 wheat squeeze, and a sale race layer adapted from sdy623 "[2842] Two Identical Agents".
...
_SQ_PARENT=agent
_SQ_UNITS=90
_SQ_REPORT=dict(squeeze_turns=0,squeeze_errors=0)
def agent(observation,configuration=None):
    action=_SQ_PARENT(observation,configuration)
    try:
        if int(observation['step'])==1 and isinstance(action,dict):
            market=action.get('market') or []
            me=int(observation['player'])
            if (len(market)>=2 and market[0]==['SELL','WHEAT',13] and market[1]==['BUY_PRODUCT','WHEAT',5]
                    and int(observation['private']['shed'].get('WHEAT',0))==0
                    and float(observation['farms'][me]['money'])>=28*_SQ_UNITS+100):
                new=[['BUY_PRODUCT','WHEAT',_SQ_UNITS],['SELL','WHEAT',_SQ_UNITS],['BUY_PRODUCT','WHEAT',5]]+[list(o) for o in market[2:]]
                action=dict(action,market=new[:10]);_SQ_REPORT['squeeze_turns']+=1
    except Exception:
        _SQ_REPORT['squeeze_errors']+=1
    return action
agent.telemetry=_SQ_REPORT
agent=globals().pop('agent')


# Sale race layer adapted from sdy623 / jaxa623, "[2842] Two Identical Agents, 90 Points Apart" (Apache-2.0).
...
def advance_sales(obs, market, future_market, telemetry=None, max_orders=10, items=PREMIUM, lookahead=None):
    ...
    try:
        ...
        if step % 24 == 23 or step >= 718:
            return market
        nxt = []
        L = LOOKAHEAD if lookahead is None else lookahead
        for off in range(1, L + 1):
            m_off = future_market(obs, off)
            if m_off:
...
class _RaceHorizons(dict):
    def get(self, key, default=None):
        v = dict.get(self, key, default)
        return _RACE_HORIZON if v is not None and v >= 4 else v


_R37_HORIZONS = _RaceHorizons()


def _race_future_market(obs, offset=1):
    try:
        st = int(obs["step"]) + offset
        if st >= 719:
            return None
        native = _IMPL.chassis.players.get(int(obs["player"]))
        route = 2 if st >= 648 else (native or {}).get("route")
        if route is None:
            return None
        return _IMPL.chassis.routes[route][st].get("market")
    except Exception:
        return None
...
_V34_REPORT = {}
def agent(observation, configuration=None):
    action = _V34_PARENT(observation, configuration)
    try:
        if isinstance(action, dict):
            market = list(action.get("market") or [])
            new = advance_sales(observation, market, _race_future_market, _V34_REPORT, lookahead=_RACE_LOOKAHEAD)
            if len(new) > 1:
                new = frontload(observation, new, None, _V34_REPORT)
            if new is not market:
                action = dict(action); action["market"] = new[:10]
    except Exception:
        _V34_REPORT["race_errors"] = _V34_REPORT.get("race_errors", 0) + 1
    return action
agent.telemetry = _V34_REPORT
agent = globals().pop('agent')
