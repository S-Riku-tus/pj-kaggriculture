"""Run paired V21 preventive-water diagnostics in the official environment."""
...
from agents.v21 import main as v21  # noqa: E402
...
def _run(seed: int, seat: int, opponent: str, replay_path: Path) -> dict[str, Any]:
    ...
    agents = [v21.agent, opponent] if seat == 0 else [opponent, v21.agent]
    env.run(agents)
    replay = env.toJSON()
    final = env.steps[-1]
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=("preventive-water", "safe-core"), required=True)
    parser.add_argument("--pairs", type=int, default=5)
    parser.add_argument("--seed", type=int, default=20262701)
    parser.add_argument("--opponent", default="starter")
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()
    v21.ENABLE_PREVENTIVE_WATER = args.mode == "preventive-water"
    created_at = datetime.now().astimezone()
    run_id = created_at.strftime("%Y%m%d_%H%M%S_%z")
    replay_dir = ROOT / "data/replays" / f"{run_id}_v21_{args.mode}_ablation"
    games = [
        _run(
            args.seed + offset,
            seat,
    ...
    output = args.output or ROOT / "data/runs" / f"v21_ablation_{args.mode}_{args.seed}.json"
