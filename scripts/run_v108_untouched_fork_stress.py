"""Stress V108 from sealed teacher triggers against foreign adaptive policies.

Both seats replay exact logged actions before the trigger.  The strong teacher
seat then becomes V11-safe or V108, while its opponent becomes local V14/V18.
Because neither continuation is the logged top policy, rewards are path and
resource diagnostics only, never top-policy counterfactuals or rating evidence.
"""
...
from scripts.run_v102_replay_forks import _logged_action  # noqa: E402
from scripts.run_v105_replay_forks import _action_difference  # noqa: E402

CONFIRMATION = ROOT / "data/analysis/v108_untouched_stage2_confirmation.json"
SOURCES = {
    "untouched_top_1_stage2_55832857": ROOT
    / "data/submissions/untouched_top_1_stage2_submission_55832857",
    "untouched_top_2_stage2_55815118": ROOT
...
def _forked(original: dict[str, Any], seat: int, fork_step: int):
    def policy(obs: Any) -> dict[str, Any]:
        step = int(obs.step)
        return _logged_action(original, step, seat) if step < fork_step else v108.agent(obs)
...
def _run(
    ...
    opponent = adaptive_v14 if source.startswith("untouched_top_1") else adaptive_v18
    _reset(v108)
    _reset(opponent)
    v108.reset_runtime_state()
    v108.ENABLE_V108_LIQUIDITY_GATE = enabled
    ...
    policies[controlled_seat] = _forked(original, controlled_seat, fork_step)
    env = make("kaggriculture", configuration={"episodeSteps": 720, "seed": seed}, debug=True)
    env.run(policies)
    replay = env.toJSON()
    rewards = [float(value or 0) for value in replay.get("rewards") or (0, 0)]
    trace = _trace(replay, controlled_seat)
    trace["active_steps"] = int(v108.v105._DAILY_ACTIVE_STEPS) if enabled else 0
    trace["changed_steps"] = int(v108.v105._DAILY_ACTIVE_STEPS) if enabled else 0
    trace["daily_activations"] = int(v108.v105._DAILY_ACTIVATIONS) if enabled else 0
    trace["emergency_fallbacks"] = int(v108.v105._DAILY_EMERGENCY_FALLBACKS) if enabled else 0
    if not enabled:
    ...
    return {
        ...
        "foreign_adaptive_opponent": "v14" if opponent is adaptive_v14 else "v18-enabled",
...
def main() -> None:
    ...
    replay_dir = ROOT / "data/replays/v108_untouched_fork_stress"
    ...
    for episode_id, fork_day in sorted(first_trigger.items()):
        ...
        controlled_seat = int(manifest["submission_seat"])
        ...
        paired.append(
            {
                ...
                "episode_id": episode_id,
                "fork_day": fork_day,
                "foreign_adaptive_opponent": candidate["foreign_adaptive_opponent"],
    ...
    result = {
        ...
        "format": "kaggriculture-v108-untouched-fork-stress-v1",
        "entry_source": str(CONFIRMATION.relative_to(ROOT)),
        "episodes": len(first_trigger),
        "fork_semantics": (
            "both seats replay exact logged actions before trigger; evaluated seat becomes "
            "V11/V108 and opponent becomes a foreign adaptive local policy"
        ),
        "opponent_semantics": (
            "top1-source uses local V14; top2-source uses enabled local V18; "
            "neither is source-equivalent"
        ),
        "purpose": "unseen-state action/resource stress only; not top-policy counterfactual or rating estimate",
        "summary": summary,
        "paired_diagnostics": paired,
        "games": games,
    }
    output = ROOT / "data/runs/v108_untouched_fork_stress.json"
