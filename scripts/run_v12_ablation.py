"""Run paired V12 component diagnostics in the official environment.

Old-agent or starter outcomes are regression context only.  Selection metrics
focus on logistics, maintenance, failures, and lower-tail economy.
"""
...
from agents.v12 import main as v12  # noqa: E402

MODES = {
    "all": (True, True, True),
    "relative": (True, False, False),
    "logistics": (False, True, True),
    "wheat": (False, True, False),
...
def _configure(mode: str) -> None:
    relative, wheat, care = MODES[mode]
    v12.ENABLE_RELATIVE_POLICY = relative
    v12.ENABLE_WHEAT_BATCH_FIX = wheat
    v12.ENABLE_LOCAL_ANIMAL_COMPLETION = care
...
def _trace(replay: dict[str, Any], seat: int) -> dict[str, float]:
    ...
    for states in replay.get("steps") or []:
        ...
        if int(obs.get("hour", 0) or 0) == 0:
            safe = v12.base._safe_observation(obs)
            if safe is not None and v12._relative_prediction(obs, *safe)["active"]:
                relative_active_days += 1
            plants = 0
            unwatered = 0
        ...
        for unit, unit_action in enumerate(current):
            ...
            if unit < len(previous) and previous[unit] and previous[unit][0] == "FEED":
                feed_follow[op if op in {"CARE", "COLLECT_FERTILIZER"} else ("MOVE" if op in MOVEMENT else op)] += 1
    ...
    return {
        ...
        "relative_active_days": float(relative_active_days),
    }


def _run(seed: int, seat: int, opponent: str, replay_path: Path) -> dict[str, Any]:
    env = make("kaggriculture", configuration={"episodeSteps": 720, "seed": seed}, debug=True)
    agents = [v12.agent, opponent] if seat == 0 else [opponent, v12.agent]
    env.run(agents)
    replay = env.toJSON()
    final = env.steps[-1]
...
def main() -> None:
    ...
    replay_dir = ROOT / "data/replays" / f"{run_id}_v12_{args.mode}_ablation"
    games = [
        _run(
            args.seed + offset,
            seat,
    ...
    payload = {
        ...
        "agent": "agents/v12/main.py",
        "mode": args.mode,
        "components": dict(zip(("relative", "wheat", "care"), MODES[args.mode], strict=True)),
        "opponent": args.opponent,
        "configuration": {"pairs": args.pairs, "seed": args.seed, "episode_steps": 720},
        "summary": summary,
        "games": games,
    }
    output = args.output or ROOT / "data/runs" / f"v12_ablation_{args.mode}_{args.seed}.json"
