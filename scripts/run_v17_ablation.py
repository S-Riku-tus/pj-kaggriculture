"""Run paired V17 herd-gate safety and trajectory diagnostics."""
...
def _gate_trace(replay: dict[str, Any], seat: int) -> dict[str, float]:
    ...
    for states in replay.get("steps") or []:
        ...
        if int(observation.get("hour", 0) or 0) != 0:
            continue
        gate = v17.policy_diagnostics(observation).get("v17_herd_gate") or {}
        reasons[str(gate.get("reason") or "missing")] += 1
        decisions[str(gate.get("decision") or "fallback")] += 1
        changed_days += bool(gate.get("changed"))
    return {
        "herd_gate_active_days": float(reasons["active"]),
        "herd_gate_freeze_days": float(decisions["freeze-owned"]),
        "herd_gate_v11_days": float(decisions["v11-herd"]),
        "herd_gate_changed_days": float(changed_days),
        "herd_gate_uncertain_days": float(reasons["uncertain"]),
        "herd_gate_fallback_days": float(sum(reasons.values()) - reasons["active"]),
    }


def _run(seed: int, seat: int, opponent: str, replay_path: Path) -> dict[str, Any]:
    env = make("kaggriculture", configuration={"episodeSteps": 720, "seed": seed}, debug=True)
    agents = [v17.agent, opponent] if seat == 0 else [opponent, v17.agent]
    env.run(agents)
    replay = env.toJSON()
    final = env.steps[-1]
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=("herd-gate", "safe-core"), required=True)
    parser.add_argument("--pairs", type=int, default=3)
    parser.add_argument("--seed", type=int, default=20262001)
    parser.add_argument("--opponent", default="starter")
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()
    v17.ENABLE_HERD_GATE = args.mode == "herd-gate"
    created_at = datetime.now().astimezone()
    run_id = created_at.strftime("%Y%m%d_%H%M%S_%z")
    replay_dir = ROOT / "data/replays" / f"{run_id}_v17_{args.mode}_ablation"
    games = [
        _run(
            args.seed + offset,
            seat,
    ...
    output = args.output or ROOT / "data/runs" / f"v17_ablation_{args.mode}_{args.seed}.json"
