"""Run paired V14 recovery diagnostics in the official environment.

Starter rewards are regression context.  The runner also records lower-tail,
maintenance, logistics, and learned recovery activity.
"""
...
def _trace(replay: dict[str, Any], seat: int) -> dict[str, float]:
    ...
    for states in replay.get("steps") or []:
        ...
        if int(obs.get("hour", 0) or 0) == 0:
            decision = v14.policy_diagnostics(obs).get("v14_winner_recovery") or {}
    ...
    return {
        ...
        "recovery_active_days": float(reasons["active"]),
        "recovery_not_behind_days": float(reasons["not-behind"]),
        "recovery_uncertain_days": float(reasons["uncertain"]),
        "recovery_ood_days": float(reasons["ood"] + reasons["low-confidence"]),
    }


def _run(seed: int, seat: int, opponent: str, replay_path: Path) -> dict[str, Any]:
    env = make("kaggriculture", configuration={"episodeSteps": 720, "seed": seed}, debug=True)
    agents = [v14.agent, opponent] if seat == 0 else [opponent, v14.agent]
    env.run(agents)
    replay = env.toJSON()
    final = env.steps[-1]
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=("recovery", "safe-core"), required=True)
    parser.add_argument("--pairs", type=int, default=2)
    parser.add_argument("--seed", type=int, default=20261501)
    parser.add_argument("--opponent", default="starter")
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()
    v14.ENABLE_WINNER_RECOVERY = args.mode == "recovery"
    created_at = datetime.now().astimezone()
    run_id = created_at.strftime("%Y%m%d_%H%M%S_%z")
    replay_dir = ROOT / "data/replays" / f"{run_id}_v14_{args.mode}_ablation"
    games = [
        _run(
            args.seed + offset,
            seat,
    ...
    output = args.output or ROOT / "data/runs" / f"v14_ablation_{args.mode}_{args.seed}.json"
