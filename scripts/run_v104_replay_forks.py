"""Run V104's latched recovery goal from frozen recent replay forks."""
...
def _reset() -> None:
    v104.v9._MISSION_LAST_STEP = -1
    v104.v9._MISSION_PREVIOUS_ACTIONS = []
...
def _forked(original: dict[str, Any], seat: int, fork_step: int):
    def policy(obs: Any) -> dict[str, Any]:
        step = int(obs.step)
        return _logged_action(original, step, seat) if step < fork_step else v104.agent(obs)
...
def _run(
    ...
    _reset()
    v104.ENABLE_LATCHED_RECOVERY = enabled
    ...
    activations = int(v104._LATCH_ACTIVATIONS)
    supported_steps = int(v104._LATCH_SUPPORTED_STEPS)
    decay_steps = int(v104._LATCH_DECAY_STEPS)
    if not enabled:
        trace["mean_normalized_change"] = 0.0
    trace["active_steps"] = supported_steps + decay_steps
    trace["changed_steps"] = supported_steps + decay_steps
    trace["latch_activations"] = activations
    trace["supported_steps"] = supported_steps
    trace["decay_steps"] = decay_steps
...
def main() -> None:
    ...
    replay_dir = ROOT / "data/replays/v104_recent_replay_forks"
        ...
        paired.append(
            {
                ...
                "latch_activations": candidate["trace"]["latch_activations"],
                "supported_steps": candidate["trace"]["supported_steps"],
                "decay_steps": candidate["trace"]["decay_steps"],
    ...
    result = {
        ...
        "format": "kaggriculture-v104-recent-replay-forks-v1",
        ...
        "purpose": "latched-goal action-effect, path, and resource diagnostic; not rating estimation",
        "summary": summary,
        "paired_diagnostics": paired,
        "games": games,
    }
    output = ROOT / "data/runs/v104_recent_replay_forks.json"
