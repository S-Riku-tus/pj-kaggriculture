"""Run paired V16 capital-WATER diagnostics."""
...
from agents.v16 import main as v16  # noqa: E402
...
def _run(seed: int, seat: int, opponent: str, replay_path: Path) -> dict[str, Any]:
    ...
    agents = [v16.agent, opponent] if seat == 0 else [opponent, v16.agent]
    env.run(agents)
    replay = env.toJSON()
    final = env.steps[-1]
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=("capital-water", "safe-core"), required=True)
    parser.add_argument("--pairs", type=int, default=2)
    parser.add_argument("--seed", type=int, default=20261901)
    parser.add_argument("--opponent", default="starter")
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()
    v16.ENABLE_CAPITAL_WATER = args.mode == "capital-water"
    created_at = datetime.now().astimezone()
    run_id = created_at.strftime("%Y%m%d_%H%M%S_%z")
    replay_dir = ROOT / "data/replays" / f"{run_id}_v16_{args.mode}_ablation"
    games = [
        _run(
            args.seed + offset,
            seat,
    ...
    output = args.output or ROOT / "data/runs" / f"v16_ablation_{args.mode}_{args.seed}.json"
