"""Run paired V54 redundant-Wheat-carrier diagnostics."""
...
from agents.v54 import main as v54  # noqa: E402
...
def _run(seed: int, seat: int, opponent: str, path: Path) -> dict[str, Any]:
    ...
    env.run([v54.agent, opponent] if seat == 0 else [opponent, v54.agent])
    replay = env.toJSON()
    final = env.steps[-1]
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=("suppress", "safe-core"), required=True)
    parser.add_argument("--pairs", type=int, default=5)
    parser.add_argument("--seed", type=int, default=20265401)
    parser.add_argument("--opponent", default="starter")
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()
    v54.ENABLE_REDUNDANT_PICKUP_SUPPRESSION = args.mode == "suppress"
    created_at = datetime.now().astimezone()
    run_id = created_at.strftime("%Y%m%d_%H%M%S_%z")
    replay_dir = ROOT / "data/replays" / f"{run_id}_v54_{args.mode}_ablation"
    games = [
        _run(
            args.seed + offset,
            seat,
    ...
    payload = {
        ...
        "configuration": {
            ...
            "seed": args.seed,
            "episode_steps": 720,
            "active_days": list(v54.ACTIVE_DAYS),
        },
        "summary": summary,
        "games": games,
    }
    output = args.output or ROOT / "data/runs" / f"v54_ablation_{args.mode}_{args.seed}.json"
