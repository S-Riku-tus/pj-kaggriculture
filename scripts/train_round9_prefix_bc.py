"""Build, train, and verify the Round9 accepted-prefix independent BC.
...
AGENT_SOURCE = ROOT / "agents" / "round9_teacher_reproduction_and_closed_loop_bc_20260923" / "a2"
if str(AGENT_SOURCE) not in sys.path:
    sys.path.insert(0, str(AGENT_SOURCE))

import common  # noqa: E402
import spatial  # noqa: E402
import runtime as prefix_runtime  # noqa: E402

EXPERIMENT = ROOT / "experiments" / "round9_teacher_reproduction_and_closed_loop_bc_20260923"
PHASE = EXPERIMENT / "a2_prefix_bc"
DATASET = PHASE / "dataset_v1"
MODEL_COLLECTION = PHASE / "models_v1"
TRAJECTORY_CONTROL = EXPERIMENT / "trajectory_control_episode_109118332_v1"
TRAJECTORY_EPISODE = 109118332
...
def _selected_entries() -> list[dict[str, Any]]:
    ...
    for row in source["files"]:
        ...
        item["selection_key"] = hashlib.sha256(f"round9-a2-prefix:{episode}".encode()).hexdigest()
...
def _sample_actor(episode: int, step: int, actor: int, token: str, quantity: int, overfit_episode: int) -> bool:
    del overfit_episode
    operation = token.split(":", 1)[0]
    if operation not in {"NORTH", "SOUTH", "EAST", "WEST", "PASS"} or quantity >= 2:
        return True
    # Movement/PASS reduction is stratified by day and hour.  It is not a
    # fixed step stride, so the 24-turn day and four-turn market phase cannot
    # alias with sampling.
    day, hour = step // 24, step % 24
    value = int(hashlib.sha256(f"round9:{episode}:{day}:{hour}:{actor}:{token}".encode()).hexdigest()[:8], 16)
    return value % 8 == 0
...
def _blocked_plants(units: list[list[Any]], seeds: dict[str, Any]) -> set[str]:
    demand = Counter(
        str(action[1]) for action in units if isinstance(action, list) and len(action) >= 2 and action[0] == "PLANT"
    )
...
def build_dataset() -> None:
    ...
    roundtrip = Counter()
    market_prefix = Counter()
    adoption_candidates: dict[str, Counter[str]] = defaultdict(Counter)
    adoption_selected: dict[str, Counter[str]] = defaultdict(Counter)
    clock_checks = Counter()
    max_actors = 0
    selected_manifest = []
    actor_provenance_path = DATASET / "actor_row_provenance.csv.gz"
    market_provenance_path = DATASET / "market_row_provenance.csv.gz"
    provenance_fields = [
        "episode",
        "record_index",
        "observation_step",
    ...
    with (
        gzip.open(actor_provenance_path, "wt", encoding="utf-8", newline="") as actor_stream,
        gzip.open(market_provenance_path, "wt", encoding="utf-8", newline="") as market_stream,
    ):
        actor_writer = csv.DictWriter(actor_stream, fieldnames=provenance_fields)
        market_writer = csv.DictWriter(market_stream, fieldnames=provenance_fields)
        ...
        for number, row in enumerate(entries, 1):
            ...
            for step in range(719):
                ...
                for actor_index, unit in enumerate(units):
                    ...
                    operation, _, item = token.partition(":")
                    phase_keys = [
                        "all",
                        f"day:{step // 24}",
                        f"hour:{step % 24}",
                        f"market_phase:{step % 4}",
                        "day0" if step < 24 else "day1plus",
                        "day_boundary" if step % 24 == 0 else "not_day_boundary",
                        "quantity_ge_2" if quantity >= 2 else "quantity_1",
                        f"operation:{operation}",
                    ]
                    for key in phase_keys:
                        adoption_candidates[f"actor:{partition}"][key] += 1
                    prior_token = (
                    ...
                    if _sample_actor(episode, step, actor_index, token, quantity, overfit_episode):
                        feature = _actor_feature(
                            ...
                            history,
                            prefix,
                            spatial.shared_spatial_features(shadow),
                        )
                        ...
                        for key in phase_keys:
                            adoption_selected[f"actor:{partition}"][key] += 1
                        actor_writer.writerow(
                            {
                                ...
                                "item": item,
                                "quantity": quantity,
                                "source_teacher": TEACHER_ID,
                                "source_label_kind": "original_teacher",
                                "partition": partition,
                            }
                        )
                ...
                for slot in range(min(10, len(orders)) + 1):
                    ...
                    for key in (
                        ...
                        adoption_candidates[f"market:{partition}"][key] += 1
                        adoption_selected[f"market:{partition}"][key] += 1
                    ...
                    if token != "EOS":
                        resolved, resolution = prefix_runtime._apply_market_prefix_order(shadow, copy.deepcopy(order))
                        market_prefix[resolution["reason"]] += 1
                        market_prefix["requested_quantity"] += quantity
                        market_prefix["accepted_quantity"] += int(resolution["accepted_quantity"])
    ...
    write_json(
        DATASET / "dataset_manifest.json",
        {
            ...
            "selection_round9": "SHA256(round9-a2-prefix:episode_id), first N within pre-existing episode split",
            ...
            "change_from_round8": (
                "actor and market features are computed from the accepted causal prefix shadow; every work and "
                "quantity-bearing actor row is retained; movement/PASS is hash-stratified by day/hour at 1/8; "
                "all market rows and EOS are retained"
            ),
            ...
            "same_turn_prefix": (
                "accepted actor/market prefix physical state plus token, raw requested quantity, "
                "target identity/generation, and fixed-engine effect"
            ),
            ...
            "market_prefix_resolution": dict(market_prefix),
            "sampling": {
                "actor": "all work and quantity>=2; movement/PASS hash-stratified 1/8 by episode/day/hour/actor/token",
                "market": "all ordered tokens and EOS",
                "fixed_stride": False,
            },
            "phase_adoption_candidates": {key: dict(value) for key, value in adoption_candidates.items()},
            "phase_adoption_selected": {key: dict(value) for key, value in adoption_selected.items()},
            "row_provenance": {
                "actor": {
                    "path": str(actor_provenance_path.relative_to(ROOT)),
                    "sha256": sha256(actor_provenance_path),
                },
                "market": {
                    "path": str(market_provenance_path.relative_to(ROOT)),
                    "sha256": sha256(market_provenance_path),
                },
            },
...
def _train_classifier(
    ...
    *,
    ...
    initial_path = output / f"{name}_initial.npz"
    if initial_path.exists():
        raise FileExistsError(f"refusing to overwrite {initial_path}")
    np.savez(
        initial_path,
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for start in range(0, len(order), batch_size):
            ...
            optimizer.step([batch_x.T @ grad_first, grad_first.sum(axis=0), grad_w2, grad_b2, grad_w3, grad_b3])
...
def train(seed: int) -> None:
    ...
    for offset, (name, group, quantity) in enumerate(
        ...
        x, y, classes = _quantity_views(group) if quantity else _token_views(group, early_repeat=1)
    ...
    write_json(
        output / "training_summary.json",
        {
            ...
            "target_task_augmentation": "none; no day-0 repetition and no fixed-phase stride",
            "old_test_status": "development diagnostic only; not a new final holdout",
            "models": results,
...
def finalize_dataset() -> None:
    """Write the manifest after arrays were completed but manifest creation failed.

    This never rewrites an array.  It independently re-reads the selected raw
    replays to recover funnel and prefix-resolution counts.
    """
    target = DATASET / "dataset_manifest.json"
    if target.exists():
        raise FileExistsError(f"refusing to overwrite {target}")
    required = [
        DATASET / f"{group}_{field}_{partition}.npy"
        for partition in PARTITION_LIMITS
        for group in ("actor", "market")
        for field in ("x", "y", "q", "episode", "step", "index" if group == "actor" else "slot")
    ]
    ...
    entries = _selected_entries()
    counts: dict[str, Counter[str]] = defaultdict(Counter)
    candidate: dict[str, Counter[str]] = defaultdict(Counter)
    ...
    for number, row in enumerate(entries, 1):
        ...
        if sha256(path) != row["sha256"]:
            raise RuntimeError(f"replay hash mismatch: {path}")
        replay = json.loads(path.read_text(encoding="utf-8"))
        seat, episode, partition = int(row["seat"]), int(row["episode_id"]), str(row["partition"])
        for step in range(719):
            observation = restore_observation(replay["steps"][step], seat, step)
            action = replay["steps"][step + 1][seat].get("action") or {}
            units = [action.get("farmer") or ["PASS"], *(action.get("hands") or [])]
            max_actors = max(max_actors, len(units))
            shadow = copy.deepcopy(observation)
            blocked = _blocked_plants(units, shadow["private"].get("seeds", {}))
            for actor_index, unit in enumerate(units):
                token = common.action_token(unit)
                ...
                for key in keys:
                    candidate[f"actor:{partition}"][key] += 1
                if _sample_actor(episode, step, actor_index, token, quantity, int(entries[0]["episode_id"])):
                    for key in keys:
                        selected[f"actor:{partition}"][key] += 1
                if common.token_action(token, quantity) != unit:
                    roundtrip[f"actor:{token}"] += 1
                _apply_teacher_prefix(shadow, actor_index, unit, blocked)
            for _slot, order in enumerate((action.get("market") or [])[:10]):
                token = common.market_token(order)
                quantity = int(order[2]) if len(order) >= 3 else 1
                if common.token_order(token, quantity) != order:
                    roundtrip[f"market:{token}"] += 1
                _resolved, resolution = prefix_runtime._apply_market_prefix_order(shadow, copy.deepcopy(order))
                market_prefix[resolution["reason"]] += 1
                market_prefix["requested_quantity"] += quantity
                market_prefix["accepted_quantity"] += int(resolution["accepted_quantity"])
        manifests.append(
        ...
        print(f"manifest {number}/{len(entries)} episode={episode} split={partition}", flush=True)
    files: dict[str, Any] = {}
    for path in sorted(DATASET.glob("*.npy")):
        array = np.load(path, mmap_mode="r", allow_pickle=False)
        files[path.stem] = {"path": str(path.relative_to(ROOT)), "sha256": sha256(path), "shape": list(array.shape)}
    for partition in PARTITION_LIMITS:
        for group, classes in (("actor", common.ACTOR_TOKENS), ("market", common.MARKET_TOKENS)):
            labels = np.load(DATASET / f"{group}_y_{partition}.npy", mmap_mode="r", allow_pickle=False)
            counts[f"{group}:{partition}"].update(str(classes[int(value)]) for value in labels)
    provenance = {}
    for name in ("actor_row_provenance.csv.gz", "market_row_provenance.csv.gz"):
        path = DATASET / name
        provenance[name] = {"path": str(path.relative_to(ROOT)), "sha256": sha256(path), "bytes": path.stat().st_size}
    write_json(
        target,
        {
            "created_at_utc": utc_now(),
            "task": "Round9 A2 accepted-prefix independent structured behavioral cloning",
            "teacher_submission": TEACHER_ID,
            "teacher_family_count": 1,
            "source_manifest": str((SOURCE_EXPERIMENT / "source_manifest.json").relative_to(ROOT)),
            "source_manifest_sha256": sha256(SOURCE_EXPERIMENT / "source_manifest.json"),
            "source_split_manifest": str((SOURCE_EXPERIMENT / "split_manifest.json").relative_to(ROOT)),
            "source_split_manifest_sha256": sha256(SOURCE_EXPERIMENT / "split_manifest.json"),
            "selection": "SHA256(round9-a2-prefix:episode_id), first N within pre-existing episode split",
            "limits": PARTITION_LIMITS,
            "old_test_status": "development diagnostic only; not a new final holdout",
            "mapping": "replay observation[t] -> saved action[t+1]",
            "format_verified": "720 saved states / 719 decisions per episode",
            "episode_separated": True,
            "feature_contract": (
                "actor features use accepted prior-actor physical prefix; "
                "market features use accepted actor plus prior-order prefix"
            ),
            "feature_schema": spatial.schema(),
            "actor_feature_width": ACTOR_WIDTH,
            "market_feature_width": MARKET_WIDTH,
            "normalization": "semantic encoder values; checkpoint mean=0 scale=1",
            "runtime_inputs_excluded": ["opponent private state", "future shop", "future action", "outcome/win/loss"],
            "sampling": {
                "actor": "all work and quantity>=2; movement/PASS hash-stratified 1/8 by episode/day/hour/actor/token",
                "market": "all ordered tokens and EOS",
                "fixed_stride": False,
            ...
            "market_prefix_resolution": dict(market_prefix),
            "max_actor_count": max_actors,
            "phase_adoption_candidates": {key: dict(value) for key, value in candidate.items()},
            "phase_adoption_selected": {key: dict(value) for key, value in selected.items()},
            "label_counts": {key: dict(value) for key, value in counts.items()},
            "episodes": manifests,
            "files": files,
            "row_provenance": provenance,
            "recovery_note": (
                "arrays and provenance completed in the original build; only manifest finalization "
                "was rerun after a Python boolean literal typo"
            ),
        },
    )
...
def _empirical_exact_input_ceiling(x: np.ndarray, y: np.ndarray) -> dict[str, Any]:
    """Compute sum_z max_a n(z,a)/N using byte-exact float32 inputs."""
    groups: dict[bytes, Counter[int]] = defaultdict(Counter)
    for row, label in zip(x, y, strict=True):
        groups[np.asarray(row, dtype=np.float32).tobytes()][int(label)] += 1
    attainable = sum(max(counts.values()) for counts in groups.values())
    conflicts = sum(sum(counts.values()) for counts in groups.values() if len(counts) > 1)
    return {
        "rows": int(len(y)),
        "exact_unique_inputs": len(groups),
        "rows_in_conflicting_exact_inputs": conflicts,
        "numerator": attainable,
        "denominator": int(len(y)),
        "ratio": attainable / len(y) if len(y) else None,
        "claim_scope": (
            "empirical ceiling for byte-exact current inputs only; not an impossibility proof for richer history"
        ),
    }


def build_trajectory_control() -> None:
    """Build every decision from one coherent recorded trajectory, without sampling."""
    dataset = TRAJECTORY_CONTROL / "dataset"
    if TRAJECTORY_CONTROL.exists():
        raise FileExistsError(f"refusing to overwrite {TRAJECTORY_CONTROL}")
    dataset.mkdir(parents=True)
    selected = [row for row in _selected_entries() if int(row["episode_id"]) == TRAJECTORY_EPISODE]
    if selected:
        row = selected[0]
    else:
        ...
        rows = [
            ...
            if int(value["episode_id"]) == TRAJECTORY_EPISODE and int(value["submission_id"]) == TEACHER_ID
        ]
        if len(rows) != 1:
            raise RuntimeError(f"expected one source row for {TRAJECTORY_EPISODE}, got {len(rows)}")
        row = {**rows[0], "partition": "trajectory_control"}
    path = ROOT / str(row["path"])
    if sha256(path) != row["sha256"]:
        raise RuntimeError(f"replay hash mismatch {path}")
    replay = json.loads(path.read_text(encoding="utf-8"))
    steps, seat = replay["steps"], int(row["seat"])
    arrays: dict[str, list[Any]] = defaultdict(list)
    counts: dict[str, Counter[str]] = defaultdict(Counter)
    roundtrip = Counter()
    prefix_resolution = Counter()
    history = common.MarketHistory()
    actor_provenance = dataset / "actor_row_provenance.csv.gz"
    market_provenance = dataset / "market_row_provenance.csv.gz"
    fields = [
        "episode",
        "record_index",
        "observation_step",
    ...
    with (
        gzip.open(actor_provenance, "wt", encoding="utf-8", newline="") as actor_stream,
        gzip.open(market_provenance, "wt", encoding="utf-8", newline="") as market_stream,
    ):
        actor_writer, market_writer = (
            csv.DictWriter(actor_stream, fieldnames=fields),
            csv.DictWriter(market_stream, fieldnames=fields),
        )
        actor_writer.writeheader()
        market_writer.writeheader()
        for step in range(len(steps) - 1):
            ...
            actor_count = 1 + len(observation["farms"][seat].get("hands", []))
            if len(units) != actor_count:
                raise RuntimeError(f"actor mismatch step={step}: {len(units)} != {actor_count}")
            prior_action = steps[step][seat].get("action") if step > 0 else {}
            prior_units = [(prior_action or {}).get("farmer") or ["PASS"], *((prior_action or {}).get("hands") or [])]
            shadow = copy.deepcopy(observation)
            blocked = _blocked_plants(units, shadow["private"].get("seeds", {}))
            prefix: list[dict[str, Any]] = []
            for actor_index, unit in enumerate(units):
                ...
                target = _target_identity(shadow, actor_index)
                feature = _actor_feature(
                    shadow, actor_index, prior_token, history, prefix, spatial.shared_spatial_features(shadow)
                )
                arrays["actor_x"].append(feature)
                arrays["actor_y"].append(common.ACTOR_TOKENS.index(token))
                arrays["actor_q"].append(quantity)
                arrays["actor_step"].append(step)
                arrays["actor_index"].append(actor_index)
                counts["actor"][token] += 1
                operation, _, item = token.partition(":")
                actor_writer.writerow(
                    {
                        "episode": TRAJECTORY_EPISODE,
                        "record_index": step + 1,
                        "observation_step": step,
                        "actor_index": actor_index,
                        "market_slot": "",
            ...
            for slot in range(min(10, len(orders)) + 1):
                ...
                token = common.market_token(order)
                quantity = int(order[2]) if len(order) >= 3 else 1
                feature = _market_feature(
                    shadow, slot, previous_token, history, spatial.shared_spatial_features(shadow)
                )
                arrays["market_x"].append(feature)
                arrays["market_y"].append(common.MARKET_TOKENS.index(token))
                arrays["market_q"].append(quantity)
                arrays["market_step"].append(step)
                arrays["market_slot"].append(slot)
                ...
                if token != "EOS":
                    if common.token_order(token, quantity) != order:
                        roundtrip[f"market:{token}"] += 1
                    _resolved, resolution = prefix_runtime._apply_market_prefix_order(shadow, copy.deepcopy(order))
                    prefix_resolution[resolution["reason"]] += 1
                previous_token = token
                if token == "EOS":
                    break
    files = {}
    for group in ("actor", "market"):
        for field, dtype in (("x", np.float32), ("y", np.int16), ("q", np.int16), ("step", np.int16)):
            value = np.asarray(arrays[f"{group}_{field}"], dtype=dtype)
            target = dataset / f"{group}_{field}.npy"
            np.save(target, value, allow_pickle=False)
            files[f"{group}_{field}"] = {
                "path": str(target.relative_to(ROOT)),
                "sha256": sha256(target),
                "shape": list(value.shape),
            }
        name = "index" if group == "actor" else "slot"
        value = np.asarray(arrays[f"{group}_{name}"], dtype=np.int16)
        target = dataset / f"{group}_{name}.npy"
        np.save(target, value, allow_pickle=False)
        files[f"{group}_{name}"] = {
            "path": str(target.relative_to(ROOT)),
            "sha256": sha256(target),
            "shape": list(value.shape),
        }
    actor_x, actor_y = np.asarray(arrays["actor_x"], dtype=np.float32), np.asarray(arrays["actor_y"], dtype=np.int64)
    market_x, market_y = (
        np.asarray(arrays["market_x"], dtype=np.float32),
        np.asarray(arrays["market_y"], dtype=np.int64),
    )
    manifest = {
        "created_at_utc": utc_now(),
        "purpose": "one coherent complete-trajectory memorization control; no generalization claim",
        "episode_id": TRAJECTORY_EPISODE,
        "source_teacher": TEACHER_ID,
        ...
        "sampling": "none; every actor and every ordered market token including EOS",
        "actor_rows": len(actor_y),
        "market_rows": len(market_y),
        "codec_roundtrip_failures": dict(roundtrip),
        "market_prefix_resolution": dict(prefix_resolution),
        "label_counts": {key: dict(value) for key, value in counts.items()},
        "exact_input_empirical_ceiling": {
            "actor_token": _empirical_exact_input_ceiling(actor_x, actor_y),
            "market_token": _empirical_exact_input_ceiling(market_x, market_y),
        },
        "files": files,
        "row_provenance": {
            "actor": {"path": str(actor_provenance.relative_to(ROOT)), "sha256": sha256(actor_provenance)},
            "market": {"path": str(market_provenance.relative_to(ROOT)), "sha256": sha256(market_provenance)},
        },
    }
    write_json(dataset / "manifest.json", manifest)


def train_trajectory_control(seed: int) -> None:
    dataset = TRAJECTORY_CONTROL / "dataset"
    output = TRAJECTORY_CONTROL / "models" / f"seed_{seed}"
    if not (dataset / "manifest.json").is_file():
        raise FileNotFoundError("build trajectory control first")
    if output.exists():
        raise FileExistsError(f"refusing to overwrite {output}")

    def load(group: str, field: str) -> np.ndarray:
        return np.load(dataset / f"{group}_{field}.npy", mmap_mode="r", allow_pickle=False)
    ...
    for offset, (name, group, quantity) in enumerate(
        ...
        base_x, token, values = load(group, "x"), load(group, "y"), load(group, "q")
        tokens = common.ACTOR_TOKENS if group == "actor" else common.MARKET_TOKENS
        if quantity:
            applicable = (
                {index for index, value in enumerate(tokens) if value.startswith(("PICKUP:", "PLACE:"))}
                if group == "actor"
                else {index for index, value in enumerate(tokens) if value not in {"EOS", "HIRE", "BUY_LAND"}}
            )
            rows = np.flatnonzero(np.isin(token, np.asarray(sorted(applicable))))
            classes = [str(value) for value in sorted(set(int(value) for value in values[rows]))]
            lookup = {int(value): index for index, value in enumerate(classes)}
            one_hot = np.zeros((len(rows), len(tokens)), dtype=np.float32)
            one_hot[np.arange(len(rows)), np.asarray(token[rows], dtype=np.int64)] = 1.0
            feature = np.concatenate((np.asarray(base_x[rows], dtype=np.float32), one_hot), axis=1)
            truth = np.asarray([lookup[int(value)] for value in values[rows]], dtype=np.int64)
        else:
            feature, truth, classes = (
                np.asarray(base_x, dtype=np.float32),
                np.asarray(token, dtype=np.int64),
                list(tokens),
            )
        x = {partition: feature for partition in ("train", "validation", "test")}
        y = {partition: truth for partition in ("train", "validation", "test")}
        results[name] = _train_classifier(
            name,
            classes,
            x,
    ...
    write_json(
        ...
        {
            "created_at_utc": utc_now(),
            "purpose": "complete single-trajectory memorization control only",
            "episode_id": TRAJECTORY_EPISODE,
            "seed": seed,
            "actual_optimizer_updates": True,
            "train_validation_test_relation": (
                "identical rows by design; this measures memorization, not generalization"
            ),
            "models": results,
        },
    )
...
def main() -> None:
    ...
    sub.add_parser("build-trajectory-control")
    trajectory_training = sub.add_parser("train-trajectory-control")
    trajectory_training.add_argument("--seed", type=int, default=20260925)
    ...
    elif args.command == "build-trajectory-control":
        build_trajectory_control()
    elif args.command == "train-trajectory-control":
        train_trajectory_control(args.seed)
    else:
        train(args.seed)
