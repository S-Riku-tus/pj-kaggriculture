"""Run paired V19 seat-aware Cow-gate closed-loop diagnostics."""
...
from agents.v19 import main as v19  # noqa: E402
...
def _gate_trace(replay: dict[str, Any], seat: int) -> dict[str, float]:
    ...
    for states in replay.get("steps") or []:
        ...
        gate = v19._herd_gate_prediction(observation, farm, opponent)
        reasons[str(gate.get("reason") or "missing")] += 1
        if not gate.get("active"):
            continue
        private = observation.get("private") or {}
        baseline = v19._SAFE_STRATEGY_TARGETS(observation, farm, opponent, private)[0]
        for animal, decision in gate["decisions"].items():
            if decision != "freeze-owned":
                continue
            freezes[animal] += 1
            owned = v19.v4._owned_animals(farm, private, animal)
            if owned < baseline[animal]:
                changed[animal] += 1
                changed_days.add(int(observation.get("day", 0) or 0))
    return {
        "herd_gate_active_steps": float(reasons["active"]),
        "herd_gate_cow_freeze_steps": float(freezes["COW"]),
        "herd_gate_sheep_freeze_steps": float(freezes["SHEEP"]),
        "herd_gate_cow_changed_steps": float(changed["COW"]),
        "herd_gate_sheep_changed_steps": float(changed["SHEEP"]),
...
def _run(seed: int, seat: int, opponent: str, replay_path: Path) -> dict[str, Any]:
    ...
    agents = [v19.agent, opponent] if seat == 0 else [opponent, v19.agent]
    env.run(agents)
    replay = env.toJSON()
    final = env.steps[-1]
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=("seat-cow-gate", "safe-core"), required=True)
    parser.add_argument("--pairs", type=int, default=3)
    parser.add_argument("--seed", type=int, default=20262501)
    parser.add_argument("--opponent", default="starter")
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()
    v19.ENABLE_SEAT_ANIMAL_GATES = args.mode == "seat-cow-gate"
    created_at = datetime.now().astimezone()
    run_id = created_at.strftime("%Y%m%d_%H%M%S_%z")
    replay_dir = ROOT / "data/replays" / f"{run_id}_v19_{args.mode}_ablation"
    games = [
        _run(
            args.seed + offset,
            seat,
    ...
    output = args.output or ROOT / "data/runs" / f"v19_ablation_{args.mode}_{args.seed}.json"
