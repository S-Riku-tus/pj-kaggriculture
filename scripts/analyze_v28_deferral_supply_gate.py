"""Test the frozen V22 opponent-supply model as a V27 deferral risk gate.

The market counterfactual showed that concurrent and next-turn opponent sales
can erase the deterministic Town-consumption advantage.  This script does not
retrain V22.  It selects a low-supply gate on Top-3 validation episodes, then
reports untouched Top-3 test and wholly external V11 behavior.
"""
...
FORMAT = "kaggriculture-v28-deferral-supply-gate-v1"
...
def _paths_and_seats() -> tuple[dict[tuple[str, str], Path], dict[tuple[str, str], int]]:
    paths: dict[tuple[str, str], Path] = {}
    seats: dict[tuple[str, str], int] = {}
    top_paths, top_sources = _episode_sources()
    for episode_id, replay_path in top_paths.items():
        for seat, source in top_sources[episode_id].items():
            paths[(source, episode_id)] = replay_path
            seats[(source, episode_id)] = seat
    for row in _manifest(V11):
        episode_id = str(row["episode_id"])
        paths[("v11", episode_id)] = _replay_path(row)
        seats[("v11", episode_id)] = int(row["submission_seat"])
    return paths, seats
...
def _select_gate(validation: list[dict[str, Any]]) -> dict[str, float]:
    ...
    event_candidates = sorted({0.0, *np.quantile(event_values, np.linspace(0.05, 1.0, 20)).tolist()})
    quantity_candidates = sorted(
        {0.0, *np.quantile(quantity_values, np.linspace(0.05, 1.0, 20)).tolist()}
    )
    ...
    return {
        ...
        "event_limit": float(selected["event_limit"]),
        "quantity_limit": float(selected["quantity_limit"]),
        "validation_selected": int(selected["selected"]),
        "validation_safe_precision": float(selected["safe_precision"]),
        "validation_safe_recall": float(selected["safe_recall"]),
        "validation_gain_p10": float(selected["logged_revenue_gain_lower_bound"]["p10"]),
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("data/analysis/v28_deferral_supply_gate.json")
    )
    ...
    if model.get("format") != MODEL_FORMAT:
        raise ValueError("a frozen V22 opponent-supply model is required")
    ...
    for index, candidate in enumerate(candidates, start=1):
        ...
        event, quantity = _raw_predictions(raw, output_count)
        output_index = output_names.index(f"h24_{candidate['item']}")
        opponent_two_turn = float(candidate["current_inventory"])
        opponent_two_turn = (
        ...
        rows.append(
            {
                **candidate,
                ...
                "supply_event_score": float(event[0, output_index]),
                "supply_quantity": float(quantity[0, output_index]),
                "uncertainty": float(np.mean(deviation)),
                "confident": float(np.mean(deviation)) <= uncertainty_limit,
            }
        )
    top_validation = [
    ...
    payload = {
        "format": FORMAT,
        "objective": "frozen Top-3 supply model gate for public Town-deferral candidates",
        ...
        "interpretation": {
            "fact": "V22 is frozen; only the low-supply gate is selected on Top-3 validation episodes",
            "limit": "V22 predicts 24-turn supply, while the hazard is a two-turn sale burst",
    ...
    print(json.dumps({"gate": gate, "reports": reports, "support": payload["support"]}, ensure_ascii=False, indent=2))
