"""Episode-split, multi-metric safety diagnostics for V110.
...
def _import_module(path: Path):
    spec = importlib.util.spec_from_file_location(
        f"_v110_diagnostic_{path.parent.name}_{abs(hash(path))}", path
    )
...
v110 = _import_module(ROOT / "agents" / "v110" / "main.py")
...
def _summary(games: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "market_overlays": dict(
            sum((Counter(game["market_overlays"]) for game in games), Counter())
        ),
        "maximum_clone_confidence": max(int(game["maximum_clone_confidence"]) for game in games),
        "second_order_games": sum(bool(game["second_order_seen"]) for game in games),
        "demand_groups": {
            key: {
                "games": len(group),
...
def _run_game(
    ...
    trace: dict[str, Any] = {
        ...
        "market_overlays": Counter(),
        "maximum_clone_confidence": 0,
        "second_order_seen": False,
        "checkpoints": {},
        "shops": [],
    }
    ...
    def candidate(obs: Any, configuration: Any = None):
        ...
        if force_fallback_step is not None and step >= force_fallback_step:
            normalized = v110.base._normalize_observation(obs)
            state = v110.base._state_for(normalized)
        ...
        action = v110.agent(obs, configuration)
        ...
        if isinstance(overlay, dict):
            trace["market_overlays"][str(overlay.get("mode", "unknown"))] += 1
        trace["maximum_clone_confidence"] = max(
            int(trace["maximum_clone_confidence"]), int(diagnostic.get("clone_confidence", 0))
        )
        trace["second_order_seen"] = bool(
            trace["second_order_seen"] or diagnostic.get("second_order_active", False)
        )
    ...
    return {
        ...
        "market_overlays": dict(trace["market_overlays"]),
        "maximum_clone_confidence": trace["maximum_clone_confidence"],
        "second_order_seen": trace["second_order_seen"],
        "shops": trace["shops"],
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/analysis/v110_diagnostics.json"))
