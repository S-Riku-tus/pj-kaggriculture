"""Research-only forced route delivery: not an eligible submission candidate."""
...
_spec = importlib.util.spec_from_file_location("_v114probe_v111", SOURCE)
...
def policy_diagnostics(obs):
    return {**base.policy_diagnostics(obs), "version": "v114probe", "research_decision": _state(obs)["decision"]}
...
def agent(obs, configuration=None):
    ...
    if base._step(obs) == DECISION_STEP and state["decision"] is None:
        original = str(
            _ORIGINAL_ROUTE(base.base.base._normalize_observation(obs), base.base.base.public_v43._V43_CONFIG)
        )
        differences = []
        ...
        state["decision"] = {
            ...
            "threshold": None,
            "research_only_forced_delivery": True,
        }
