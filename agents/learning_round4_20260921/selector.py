"""Pure-Python inference for the trained strategy skill selector."""
...
class SkillSelector:
    def __init__(self, path: str | Path) -> None:
        ...
        if not self.path.is_file():
            raise FileNotFoundError(f"required Round4 strategy model is missing: {self.path}")
        model = json.loads(self.path.read_text(encoding="utf-8"))
        if model.get("format") != "round4-softmax-skill-selector-v1":
            raise RuntimeError("Round4 strategy model format mismatch")
        if tuple(model.get("feature_names") or ()) != FEATURE_NAMES:
            raise RuntimeError("Round4 strategy feature schema mismatch")
        ...
        if not (
            ...
            raise RuntimeError("Round4 strategy model shape mismatch")
