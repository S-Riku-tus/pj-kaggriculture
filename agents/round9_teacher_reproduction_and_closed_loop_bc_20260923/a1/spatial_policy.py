"""Learned policy interface for the Round8 position-sensitive BC checkpoints."""
...
def _shared_features(observation: Mapping[str, Any]) -> np.ndarray:
    ...
    cached = _shared_cache.get(seat)
    if cached is None or cached[0] != step:
        cached = (step, shared_spatial_features(observation))
        _shared_cache[seat] = cached
    return cached[1]
