"""Round9 A1: frozen Round8 weights with prefix-correct legality/quantities.
...
def _decode_actor(
    ...
    *,
    ...
    # A1 deliberately keeps the frozen model input identical to Round8.  Only
    # legality, available quantity, and target identity use the accepted
    # same-turn prefix state.
...
def _decode_market(
    observation: Mapping[str, Any], history: Any, units: list[list[Any]]
    ...
    shed = _simulate_shed_after_actors(observation, units)
    for slot in range(10):
        ...
        for candidate_value, _score in ranking:
            ...
            if candidate.startswith("SELL:"):
                ...
                if available <= 0:
                    rejected.append({"token": candidate, "reason": "shed_empty"})
                    _policy._stats["blocked_market_orders"] += 1
                    continue
                candidate_quantity = min(candidate_quantity, available)
            elif (
                candidate.startswith(("BUY_PRODUCT:", "BUY_ANIMAL:")) and sum(shed.values()) + candidate_quantity > 100
            ):
                rejected.append({"token": candidate, "reason": "shed_capacity"})
                _policy._stats["blocked_market_orders"] += 1
                continue
            ...
            if candidate_cost > guaranteed_money:
                rejected.append({"token": candidate, "reason": "guaranteed_funds"})
                _policy._stats["blocked_market_orders"] += 1
                continue
        ...
        if token.startswith("SELL:"):
            ...
            guaranteed_money += quantity  # fixed engine has a $1 floor
        else:
...
def _decode_ledger(observation: Mapping[str, Any], *, full_contract: bool) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    market, market_trace = _decode_market(observation, history, units)
...
def _resolve_market(shadow: dict[str, Any], requested: list[list[Any]]) -> tuple[list[list[Any]], list[dict[str, Any]]]:
    seat = _number(shadow.get("player"))
    farm = shadow["farms"][seat]
    shed = shadow["private"]["shed"]
    seeds = shadow["private"]["seeds"]
    guaranteed_money = float(farm.get("money", 0))
    hired = lands = 0
    bought: dict[str, int] = {}
    final: list[list[Any]] = []
    trace: list[dict[str, Any]] = []
    for slot, order in enumerate(requested[:10]):
        before_money = guaranteed_money
        resolved: list[Any] | None = None
        reason = "schema_rejected"
        if isinstance(order, list) and order:
            op = str(order[0])
            if op in {"HIRE", "BUY_LAND"}:
                cost = _policy._market_cost(shadow, op, 1, hired, lands, bought)
                if cost <= guaranteed_money:
                    resolved, reason = [op], "guaranteed_funds"
                    guaranteed_money -= cost
                    farm["money"] = guaranteed_money
                    if op == "HIRE":
                        hired += 1
                    else:
                        lands += 1
            elif len(order) >= 2 and op in {"SELL", "BUY_SEED", "BUY_PRODUCT", "BUY_ANIMAL"}:
                item = str(order[1])
                quantity = max(1, _number(order[2], 1) if len(order) >= 3 else 1)
                if op == "SELL":
                    quantity = min(quantity, max(0, _number(shed.get(item))))
                    if quantity > 0:
                        resolved, reason = [op, item, quantity], "quantity_clipped_to_actor_poststate_shed"
                        shed[item] = _number(shed.get(item)) - quantity
                        guaranteed_money += quantity
                        farm["money"] = guaranteed_money
                else:
                    token = f"{op}:{item}"
                    if op in {"BUY_PRODUCT", "BUY_ANIMAL"}:
                        quantity = min(quantity, max(0, 100 - sum(_number(v) for v in shed.values())))
                    cost = (
                        _policy._market_cost(shadow, token, quantity, hired, lands, bought)
                        if quantity > 0
                        else float("inf")
                    )
                    if quantity > 0 and cost <= guaranteed_money:
                        resolved, reason = [op, item, quantity], "guaranteed_funds_and_capacity"
                        guaranteed_money -= cost
                        farm["money"] = guaranteed_money
                        if op == "BUY_SEED":
                            seeds[item] = _number(seeds.get(item)) + quantity
                        else:
                            shed[item] = _number(shed.get(item)) + quantity
                            if op == "BUY_PRODUCT":
                                bought[item] = bought.get(item, 0) + quantity
        ...
        trace.append(
            {
                "slot": slot,
                "requested": copy.deepcopy(order),
                "final": copy.deepcopy(resolved),
                "reason": reason,
                "money_before": before_money,
                "guaranteed_money_after": guaranteed_money,
            }
        )
    return final, trace
...
def diagnostics() -> dict[str, Any]:
    return {
        "arm": "round9_a1_frozen_weights_prefix_legality",
        "plans": _plans,
        "trace": _trace,
