"""B2 overlay: predict market-impact supply and reorder only C0 SELL blocks."""
...
def _load_c0() -> Any:
    ...
    spec = importlib.util.spec_from_file_location(f"_round2_b2_c0_{id(source)}", source)
...
MODEL = SavedMLP(_module_dir() / "b2_model.npz") if MODE in {"learned", "history_shuffle"} else None
SIMPLE_PATH = _module_dir() / "b2_simple.json"
SIMPLE = json.loads(SIMPLE_PATH.read_text(encoding="utf-8")) if SIMPLE_PATH.is_file() else {}
...
def reset_runtime_state() -> None:
    ...
    _history, _previous_market, _previous_probe = {}, {}, {}
    _stats = {
        "model_loads": int(MODE in {"learned", "history_shuffle"}),
        "base_calls": 0,
        "inference_calls": 0,
        "candidate_sets": 0,
        "action_changes": 0,
...
def _call_base(observation: Mapping[str, Any], configuration: Any) -> Any:
    try:
        values = inspect.signature(base.agent).parameters.values()
        accepts_configuration = any(
            value.kind in {value.VAR_POSITIONAL, value.VAR_KEYWORD} for value in values
        ) or len(list(values)) >= 2
...
def _predict(observation: Mapping[str, Any], history: MarketHistory) -> dict[tuple[str, int], tuple[float, float]]:
    ...
    if MODE == "frequency":
        ...
        return result
    if MODEL is None:
        raise RuntimeError("B2 learned mode requires b2_model.npz")
    features = state_features(observation, MarketHistory() if MODE == "history_shuffle" else history)
    outputs = MODEL.logits(features)
    width = len(PRODUCTS) * len(HORIZONS)
    probability = 1.0 / (1.0 + np.exp(-np.clip(outputs[:width], -30.0, 30.0)))
    quantity = np.maximum(0.0, np.expm1(np.clip(outputs[width : 2 * width], 0.0, 8.0)))
    for offset, key in enumerate((item, horizon) for item in PRODUCTS for horizon in HORIZONS):
        result[key] = (float(probability[offset]), float(quantity[offset]))
    _stats["inference_calls"] += 1
    return result


def sell_blocks(orders: Sequence[Sequence[Any]], risk: Mapping[str, float]) -> list[list[list[Any]]]:
...
def score_sell_candidate(
    ...
    for slot in range(max(len(own_orders), len(opponent))):
        ...
        quantities = [int(order[2]) if order and len(order) >= 3 and order[0] == "SELL" else 0 for order in pair]
        for unit in range(max(quantities, default=0)):
            quoted = [
                (str(order[1]), market_price(str(order[1]), inventory[str(order[1])]))
...
def agent(observation: Mapping[str, Any], configuration: Any = None) -> dict[str, Any]:
    ...
    candidates = sell_blocks(control["market"], risk)
    _stats["candidate_sets"] += 1
    scores = [score_sell_candidate(observation, candidate, prediction) for candidate in candidates]
