"""Kaggriculture V17: winner-trained recovery herd gate on V14.

The learned layer makes one bounded macro decision: while behind, should the
agent keep V11's planned Cow/Sheep target or stop buying at the already-owned
herd? It cannot select an arbitrary herd size. V14's winner-manifold distance
and this gate's own uncertainty check are mandatory. V14/V11 still control
crops, workers, land, routing, feed, legality, inventory, market feasibility,
and all action execution.
"""
    ...
    MODULE_DIR = next(
        (
            ...
            if (
                ...
                and (candidate / "herd_gate_model.json").is_file()
            )
...
def _load_v14_module():
    ...
    spec = importlib.util.spec_from_file_location("_kaggriculture_v17_safe_core", source)
...
MODEL_FORMAT = "kaggriculture-v17-herd-gate-v1"
ENABLE_HERD_GATE = False


def _load_model() -> dict[str, Any] | None:
    path = MODULE_DIR / "herd_gate_model.json"
    try:
...
def _herd_gate_prediction(obs: Any, farm: Any, opponent_farm: Any) -> dict[str, Any]:
    if not ENABLE_HERD_GATE or HERD_MODEL is None:
    ...
    manifold_confidence, distance = v14._distance_confidence(features, day)
    minimum_confidence = float(HERD_MODEL["manifold_min_confidence"])
    if manifold_confidence < minimum_confidence:
        return {
            "active": False,
            "reason": "ood",
            "money_gap_ratio": money_gap_ratio,
...
def _strategy_targets(
    ...
    pastures = max(
        v8._pasture_count(farm),
        animals["COW"] + animals["SHEEP"],
    )
    return animals, crops, hands, land, weights, pastures


# V14 resolves its strategy function at call time. Install one stable wrapper;
# every field and market action still passes through V14/V11's safe executor.
...
def policy_diagnostics(obs: Any) -> dict[str, Any]:
    ...
    result["v17_herd_gate"] = prediction
    result["v17_herd_targets"] = {
        "animals": targets[0],
        "pastures": targets[5],
    }
    return result
